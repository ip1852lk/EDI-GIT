 ...
 *
 * The followings are the available columns in table 'pl1_plant':
 * @property string $id
 * @property string $pl1_code
 ...
 * @property AccountRep[] $accountReps
 * @property Customer $customer
 * @property Rack[] $racks
 * @property Profile $cprofile
 * @property Profile $mprofile
 */
class Plant extends JActiveRecord
{
    ...
    public function rules()
    {
        ...
        return array(
            array('pl1_code, pl1_name, cu1_id', 'required', 'on' => 'insert, update'),
            array('delete_flag, update_flag', 'numerical', 'integerOnly' => true),
            array('pl1_code', 'length', 'max' => 10),
            array('pl1_name', 'length', 'max' => 250),
            array('cu1_id, created_by, modified_by', 'length', 'max' => 20),
            array('created_on, modified_on', 'safe'),
            // The following rule is used by search().
            // Please remove those attributes that should not be searched.
            array('id, pl1_code, pl1_name, cu1_id, cu1_search, created_by, created_on, modified_by, modified_on, delete_flag, update_flag, cprofile_search, mprofile_search', 'safe', 'on' => 'search'),
            // Rules for relations
            array('pl1_name', 'required', 'on' => 'relation', 'message' => Yii::t('app', Yii::app()->params['plantDisplayLabel1'].' cannot be blank.')),
        );
    }
    ...
    public function relations()
    {
        ...
        return array(
            'accountReps' => array(self::HAS_MANY, 'AccountRep', 'pl1_id'),
            'customer' => array(self::BELONGS_TO, 'Customer', 'cu1_id'),
            'racks' => array(self::HAS_MANY, 'Rack', 'pl1_id'),
    ...
    public function attributeLabels()
    {
        return array(
            'id' => Yii::t('app', 'ID'),
            'pl1_code' => Yii::t('app', 'Code'),
            'pl1_name' => Yii::t('app', Yii::app()->params['plantDisplayLabel1'].' Name'),
    ...
    {
        $alias = Plant::model()->getTableAlias(false, false);
        $scope = parent::defaultScope();
        $scope->order = $alias . '.cu1_id, ' . $alias . '.pl1_code';
    ...
    public function scopes()
    {
        ...
        return array(
            ...
            'relation' => array(
                'select' => $alias . '.id, ' . $alias . '.pl1_name',
                'together' => true,
                'with' => array('customer', 'cprofile', 'mprofile',),
                'order' => $alias . '.cu1_id, ' . $alias . '.pl1_code',
            ),
        );
    }
    ...
    public static function getListData($criteria = null, $addEmptyItem = true)
    {
        ...
        $models = Plant::model()->relation()->cache(10 * 60)->findAll($criteria);
        $rtv = array();
        if ($addEmptyItem) $rtv['0'] = '';
        foreach ($models as $model) {
            if (isset($model->id))
                $rtv[$model->id] = $model->pl1_name;
        }
        return $rtv;
    }
    ...
    public function search()
    {
        ...
        $criteria->with = array('customer', 'cprofile', 'mprofile',);
        $criteria->compare($alias . '.id', $this->id);
        $criteria->compare($alias . '.pl1_code', $this->pl1_code, true);
        $criteria->compare($alias . '.pl1_name', $this->pl1_name, true);
       ...
       $pageSize = Yii::app()->user->getState('pageSize', Yii::app()->params['pageSize']);
        return new CActiveDataProvider($this, array(
            ...
            'sort' => array(
                'attributes' => array(
                    'cu1_search' => array(
                        'asc' => 'customer.cu1_customer',
                        'desc' => 'customer.cu1_customer DESC',
                    ),
    ...
    public function export($stream)
    {
        return Yii::app()->export->exportData($this, array(
            ...
            'stream' => $stream,
            'fileName' => Yii::t('app', Yii::app()->params['plantDisplayLabel1']).'-'.date('Y-m-d-H-i-s'),
            'extensionType' => 'Excel5',
            'columns' => array(
                array(
                    'field' => 'pl1_code',
                    'label' => $this->getAttributeLabel('pl1_code'),
                ...
                array(
                    'field' => 'pl1_name',
                    'label' => $this->getAttributeLabel('pl1_name'),
                    'type' => PHPExcel_Cell_DataType::TYPE_STRING,
                    'value' => null,
                    'width' => 25,
                    'itemAlias' => null,
