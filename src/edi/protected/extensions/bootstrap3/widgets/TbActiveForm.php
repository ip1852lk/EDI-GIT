...
class TbActiveForm extends CActiveForm
{
    ...
    /**
     * Whether to render a required symbol after each required label.
     * @var bool
     */
    public $showRequiredSymbol = true;
    ...
    protected function horizontalGroup(&$fieldData, &$model, &$attribute, &$options)
    {
        ...
        if (isset($options['labelOptions']['class'])) {
            if (!(preg_match("/^.*(\b[c][o][l]\-[xsml][smdg]\-([4-9]|[1][0-2])\b).*$/", $options['labelOptions']['class']) > 0)) {
        ...
        if (isset($options['label'])) {
            if (!empty($options['label'])) {
                ...
                if (isset($options['labelOptions']) && isset($options['labelOptions']['class'])) {
                    echo '<span class="'.$options['labelOptions']['class'].'"></span>';
                } else {
        ...
        // TODO
        if (isset($wrapperHtmlOptions['class'])) {
            if (!(preg_match("/^.*(\b[c][o][l]\-[xsml][smdg]\-[1][0-2]\b).*$/", $wrapperHtmlOptions['class']) > 0)) {
                $this->addCssClass($wrapperHtmlOptions, 'col-md-9');
            }
        } else {
            $this->addCssClass($wrapperHtmlOptions, 'col-md-9');
        }
    ...
    {
        $inputGroupOptions = isset($prependOptions['inputGroupOptions']) ? $prependOptions['inputGroupOptions'] : array();
        if (!empty($prependText))
            self::addCssClass($inputGroupOptions, $this->prependCssClass);
        if (!empty($appendText))
            self::addCssClass($inputGroupOptions, $this->appendCssClass);
        if (isset($prependOptions['inputGroupOptions']))
            unset($prependOptions['inputGroupOptions']);
        echo CHtml::tag($this->addOnWrapperTag, $inputGroupOptions, false, false);
        if (!empty($prependText)) {
