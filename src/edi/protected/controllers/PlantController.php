...
class PlantController extends RController
{
    /**
     * REQUIRED
    ...
    public function actionCreate()
    {
        ...
        $this->performAjaxValidation(array($model, $customer));
        if (isset($_POST['Plant']) && isset($_POST['Customer'])) {
            ...
            if ($model->validate() && $customer->validate()) {
                if ($model->save()) {
                    ...
                    throw new CHttpException(500, Yii::t('app', 'Plant cannot be created.'));
            }
        }
        ...
        $this->render('//plant/create', array(
            'model' => $model,
            'customer' => $customer,
        ...
        if (isset($_POST['Plant']) && isset($_POST['Customer'])) {
            ...
            if ($model->validate() && $customer->validate()) {
                if ($model->save()) {
                    ...
                    throw new CHttpException(500, Yii::t('app', 'Plant cannot be saved.'));
            }
        }
        $this->render('//plant/update', array(
            'model' => $model,
            'customer' => $customer,
            'dependency' => (isset($_GET['dependency'])?$_GET['dependency']:null), 
            'dependencyTabIndex' => (isset($_GET['dependencyTabIndex'])?$_GET['dependencyTabIndex']:null), 
    ...
    public function actionEdit()
    {
        if (isset($_POST['pk']) && isset($_POST['name']) && isset($_POST['value'])) {
            $model = Plant::model()->findByPk((int)$_POST['pk']);
            if (isset($model)) {
    ...
    {
        $dataProvider = new CActiveDataProvider('Plant');
        $this->render('//plant/blog', array(
    ...
    public function actionIndex()
    {
        ...
        if (isset($_GET['export'])) {
            if (Yii::app()->user->checkAccess('Plant.Export')) {
                if (isset($_REQUEST['ajax']) && $_REQUEST['ajax'] === 'plant-grid') {
                    header('Content-type: application/json');
                    $file = $model->export(false);
                    if ($file === false) {
                        $status = '500';
                        $statusText = 'fail';
                        $body = Yii::t('app', 'Plant data cannot be exported.');
                    } else {
                        $status = '200';
                        $statusText = 'success';
                        $body = $file;
                    ...
                    if (!$model->export(true)) {
                        Yii::log('Plant data cannot be exported.', CLogger::LEVEL_ERROR);
                        throw new CHttpException(500, Yii::t('app', 'Plant data cannot be exported.'));
                    }
                }
            } else {
                Yii::log('This user is not permitted to export Plant data.', CLogger::LEVEL_ERROR);
                throw new CHttpException(401, Yii::t('app', 'You are not fully authorized to export Plant data.'));
            }
        }
        $this->render('//plant/index', array(
            'model' => $model,
        ));
    }
    ...
    public function actionDependency()
    {
        if (isset($_GET['dependency']) && isset($_GET['parentPk']) && isset($_GET['parentId'])) {
            ...
            if (isset($_GET['export'])) {
                if (Yii::app()->user->checkAccess('Plant.Export')) {
                    if (isset($_REQUEST['ajax']) && $_REQUEST['ajax'] === 'plant-grid-'.$_GET['dependencyTabDropdownIndex']) {
                        header('Content-type: application/json');
                        $file = $model->export(false);
                        if ($file === false) {
                            $status = '500';
            ...
            $this->renderPartial('//plant/_grid', array(
                'model' => $model,
    ...
    {
        header('Content-type: application/json');
        if (Yii::app()->user->checkAccess('Plant.AjaxGet')) {
            ...
            if (isset($_GET['pl1_id']))
                $criteria->compare($alias.'.id', $_GET['pl1_id']);
            if (isset($_GET['cu1_id']))
                $criteria->compare($alias.'.cu1_id', $_GET['cu1_id']);
            $return = array(
                'status' => '200',
                'statusText' => 'success',
                'body' => Plant::getListData($criteria),
            );
        } else {
            $return = array(
                'status' => '401',
    ...
    {
        if (isset($_REQUEST['ajax']) && $_REQUEST['ajax'] === 'plant-form') {
