...
class ProductFamilyController extends RController
{
    /**
     * REQUIRED
    ...
    {
        $this->render('//productFamily/view', array(
    ...
    public function actionCreate()
    {
        ...
        if (isset($_POST['ProductFamily'])) {
            ...
            if ($model->validate()) {
                if ($model->save()) {
                    ...
                    throw new CHttpException(500, Yii::t('app', 'ProductFamily cannot be created.'));
            }
        }
        ...
        $this->render('//productFamily/create', array(
            'model' => $model,
        ...
        if (isset($_POST['ProductFamily'])) {
            ...
            if ($model->validate()) {
                if ($model->save()) {
                    ...
                    throw new CHttpException(500, Yii::t('app', 'ProductFamily cannot be saved.'));
            }
        }
        $this->render('//productFamily/update', array(
            'model' => $model,
            'dependency' => (isset($_GET['dependency'])?$_GET['dependency']:null), 
            'dependencyTabIndex' => (isset($_GET['dependencyTabIndex'])?$_GET['dependencyTabIndex']:null), 
            'dependencyTabDropdownIndex' => (isset($_GET['dependencyTabDropdownIndex'])?$_GET['dependencyTabDropdownIndex']:null),
    ...
    public function actionEdit()
    {
        if (isset($_POST['pk']) && isset($_POST['name']) && isset($_POST['value'])) {
            $model = ProductFamily::model()->findByPk((int)$_POST['pk']);
            if (isset($model)) {
    ...
    {
        $dataProvider = new CActiveDataProvider('ProductFamily');
        $this->render('//productFamily/blog', array(
    ...
    public function actionIndex()
    {
        ...
        if (isset($_GET['export'])) {
            if (Yii::app()->user->checkAccess('ProductFamily.Export')) {
                if (isset($_REQUEST['ajax']) && $_REQUEST['ajax'] === 'product-family-grid') {
                    header('Content-type: application/json');
                    $file = $model->export(false);
                    if ($file === false) {
                        $status = '500';
                        $statusText = 'fail';
                        $body = Yii::t('app', 'ProductFamily data cannot be exported.');
                    } else {
                        $status = '200';
                        $statusText = 'success';
                        $body = $file;
                    ...
                    if (!$model->export(true)) {
                        Yii::log('ProductFamily data cannot be exported.', CLogger::LEVEL_ERROR);
                        throw new CHttpException(500, Yii::t('app', 'ProductFamily data cannot be exported.'));
                    }
                }
            } else {
                Yii::log('This user is not permitted to export ProductFamily data.', CLogger::LEVEL_ERROR);
                throw new CHttpException(401, Yii::t('app', 'You are not fully authorized to export ProductFamily data.'));
            }
        }
        $this->render('//productFamily/index', array(
            'model' => $model,
        ));
    }
    ...
    public function actionDependency()
    {
        if (isset($_GET['dependency']) && isset($_GET['parentPk']) && isset($_GET['parentId'])) {
            ...
            if (isset($_GET['export'])) {
                if (Yii::app()->user->checkAccess('ProductFamily.Export')) {
                    if (isset($_REQUEST['ajax']) && $_REQUEST['ajax'] === 'product-family-grid-'.$_GET['dependencyTabDropdownIndex']) {
                        header('Content-type: application/json');
                        $file = $model->export(false);
                        if ($file === false) {
                            $status = '500';
            ...
            $this->renderPartial('//productFamily/_grid', array(
                'model' => $model,
    ...
    {
        header('Content-type: application/json');
        if (Yii::app()->user->checkAccess('ProductFamily.AjaxGet')) {
            ...
            $return = array(
                'status' => '200',
                'statusText' => 'success',
                'body' => ProductFamily::getListData($criteria),
            );
        } else {
            $return = array(
                'status' => '401',
    ...
    {
        if (isset($_REQUEST['ajax']) && $_REQUEST['ajax'] === 'product-family-form') {
