...
class CompanyController extends RController
{
    ...
    public function actionCreate()
    {
        ...
        if (isset($_POST['Company'])) {
            ...
            $logo = CUploadedFile::getInstance($model, 'co1_logo');
            if ($logo) {
                $logoPath = '/img/logo/' . $model->co1_code;
                $fullPath = Yii::app()->basePath . '/..' . $logoPath;
                if (!file_exists($fullPath))
                    mkdir($fullPath, 0755, true);
                $logoPath .= '/' . $logo->name;
                $logo->saveAs(Yii::app()->basePath . '/..' . $logoPath);
                $model->co1_logo = $this->createUrl($logoPath);
            }
            if ($model->save()) {
                ...
                throw new CHttpException(500, Yii::t('app', 'Company cannot be created.'));
        }
    ...
    public function actionUpdate($id)
    {
        ...
        if (isset($_POST['Company'])) {
            ...
            if ($model->save()) {
                ...
                throw new CHttpException(500, Yii::t('app', 'Company cannot be saved.'));
        }
    ...
    public function actionDelete($id)
    {
        if (Yii::app()->request->isPostRequest) {
            ...
            if (!isset($_REQUEST['ajax']))
                Yii::app()->user->setFlash('success', Yii::t('app', '<span class="label label-success">DELETED</span> <span class="label label-warning">:record</span> is deleted successfully.', array(':record' => $model->co1_code)));
            else
                echo Yii::t('app', '<div class="alert alert-success"><button type="button" class="close" data-dismiss="alert">&times;</button><span class="label label-success">DELETED</span> <span class="label label-warning">:record</span> is deleted successfully.</div>', array(':record' => $model->co1_code));
            // Redirect
            if (isset($_REQUEST['ajax']))
                Yii::app()->end();
    ...
    public function actionIndex()
    {
        ...
        if (isset($_GET['export'])) {
            if (Yii::app()->user->checkAccess('Company.Export')) {
                if (isset($_REQUEST['ajax']) && $_REQUEST['ajax'] === 'company-grid') {
                    header('Content-type: application/json');
                    $file = $model->export(false);
                    if ($file === false) {
                        $status = '500';
                        $statusText = 'fail';
                        $body = Yii::t('app', 'Company data cannot be exported.');
                    } else {
                        $status = '200';
                        $statusText = 'success';
                        $body = $file;
                    ...
                    if (!$model->export(true)) {
                        Yii::log('Company data cannot be exported.', CLogger::LEVEL_ERROR);
                        throw new CHttpException(500, Yii::t('app', 'Company data cannot be exported.'));
                    }
                }
            } else {
                Yii::log('This user is not permitted to export Company data.', CLogger::LEVEL_ERROR);
                throw new CHttpException(401, Yii::t('app', 'You are not fully authorized to export Company data.'));
            }
        }
    ...
    public function actionDependency()
    {
        if (isset($_GET['dependency']) && isset($_GET['parentPk']) && isset($_GET['parentId'])) {
            ...
            if (isset($_GET['export'])) {
                if (Yii::app()->user->checkAccess('Company.Export')) {
                    if (isset($_REQUEST['ajax']) && $_REQUEST['ajax'] === 'company-grid-'.$_GET['dependencyTabDropdownIndex']) {
                        header('Content-type: application/json');
                        $file = $model->export(false);
                        if ($file === false) {
                            $status = '500';
            ...
            $this->renderPartial('//company/_grid', array(
                'model' => $model,
    ...
    public function actionRelation()
    {
        ...
        if (isset($_GET['Company']))
            $model->attributes = $_GET['Company'];
        if (isset($_GET['co1_code']))
            $model->co1_code = $_GET['co1_code'];
        if (isset($_GET['co1_name']))
            $model->co1_name = $_GET['co1_name'];
        if (isset($_GET['co1_type']))
            $model->co1_type = (int)$_GET['co1_type'];
        $this->renderPartial('//company/_grid', array(
            'model' => $model,
            'relation' => true,
    ...
    {
        header('Content-type: application/json');
        if (Yii::app()->user->checkAccess('Company.AjaxGet')) {
            ...
            if (isset($_GET['co1_code']))
                $criteria->compare($alias.'.co1_code', $_GET['co1_code'], true);
            if (isset($_GET['co1_name']))
                $criteria->compare($alias.'.co1_name', $_GET['co1_name'], true);
            if (isset($_GET['co1_type']))
                $criteria->compare($alias.'.co1_type', $_GET['co1_type']);
            $return = array(
                'status' => '200',
                'statusText' => 'success',
                'body' => Company::getListData($criteria),
            );
        } else {
            $return = array(
                'status' => '401',
    ...
    {
        if (isset($_REQUEST['ajax']) && $_REQUEST['ajax'] === 'company-form') {
