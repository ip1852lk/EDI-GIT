...
class P21AddressController extends RController
{
    /**
     * REQUIRED
    ...
    {
        $this->render('//p21Address/view', array(
    ...
    {
        $model = new P21Address();
        $this->performAjaxValidation(array($model));
        if (isset($_POST['P21Address'])) {
            $model->attributes = $_POST['P21Address'];
            if ($model->validate()) {
                if ($model->save()) {
                    ...
                    throw new CHttpException(500, Yii::t('app', 'P21Address cannot be created.'));
            }
        }
        ...
        $this->render('//p21Address/create', array(
            'model' => $model,
        ...
        if (isset($_POST['P21Address'])) {
            ...
            if ($model->validate()) {
                if ($model->save()) {
                    ...
                    throw new CHttpException(500, Yii::t('app', 'P21Address cannot be saved.'));
            }
        }
        $this->render('//p21Address/update', array(
            'model' => $model,
            'dependency' => (isset($_GET['dependency'])?$_GET['dependency']:null), 
            'dependencyTabIndex' => (isset($_GET['dependencyTabIndex'])?$_GET['dependencyTabIndex']:null), 
            'dependencyTabDropdownIndex' => (isset($_GET['dependencyTabDropdownIndex'])?$_GET['dependencyTabDropdownIndex']:null),
    ...
    public function actionEdit()
    {
        if (isset($_POST['pk']) && isset($_POST['name']) && isset($_POST['value'])) {
            $model = P21Address::model()->findByPk((int)$_POST['pk']);
            if (isset($model)) {
    ...
    {
        $dataProvider = new CActiveDataProvider('P21Address');
        $this->render('//p21Address/blog', array(
    ...
    public function actionIndex()
    {
        ...
        $model = new P21Address('search');
        $model->unsetAttributes();  // clear any default values
        if (isset($_GET['P21Address']))
            $model->attributes = $_GET['P21Address'];
        if (isset($_GET['export'])) {
            if (Yii::app()->user->checkAccess('P21Address.Export')) {
                if (isset($_REQUEST['ajax']) && $_REQUEST['ajax'] === 'p21-address-grid') {
                    header('Content-type: application/json');
                    $file = $model->export(false);
                    if ($file === false) {
                        $status = '500';
                        $statusText = 'fail';
                        $body = Yii::t('app', 'P21Address data cannot be exported.');
                    } else {
                        $status = '200';
                        $statusText = 'success';
                        $body = $file;
                    ...
                    if (!$model->export(true)) {
                        Yii::log('P21Address data cannot be exported.', CLogger::LEVEL_ERROR);
                        throw new CHttpException(500, Yii::t('app', 'P21Address data cannot be exported.'));
                    }
                }
            } else {
                Yii::log('This user is not permitted to export P21Address data.', CLogger::LEVEL_ERROR);
                throw new CHttpException(401, Yii::t('app', 'You are not fully authorized to export P21Address data.'));
            }
        }
        $this->render('//p21Address/index', array(
            'model' => $model,
        ));
    }
    ...
    public function actionDependency()
    {
        if (isset($_GET['dependency']) && isset($_GET['parentPk']) && isset($_GET['parentId'])) {
            ...
            if (isset($_GET['export'])) {
                if (Yii::app()->user->checkAccess('P21Address.Export')) {
                    if (isset($_REQUEST['ajax']) && $_REQUEST['ajax'] === 'p21-address-grid-'.$_GET['dependencyTabDropdownIndex']) {
                        header('Content-type: application/json');
                        $file = $model->export(false);
                        if ($file === false) {
                            $status = '500';
            ...
            $this->renderPartial('//p21Address/_grid', array(
                'model' => $model,
    ...
    {
        header('Content-type: application/json');
        if (Yii::app()->user->checkAccess('P21Address.AjaxGet')) {
            $alias = P21Address::model()->getTableAlias(false, false);
            ...
            $return = array(
                'status' => '200',
                'statusText' => 'success',
                'body' => P21Address::getListData($criteria),
            );
        } else {
            $return = array(
                'status' => '401',
    ...
    {
        $model = P21Address::model()->findByPk($id);
        if ($model === null)
            throw new CHttpException(404, Yii::t('app', 'The requested page does not exist.'));
        return $model;
    }
    ...
    {
        if (isset($_REQUEST['ajax']) && $_REQUEST['ajax'] === 'p21-address-form') {
