...
class LogController extends RController
{
        ...
        if (isset($_POST['Log'])) {
            ...
            if ($model->validate()) {
                if ($model->save()) {
                    ...
                    throw new CHttpException(500, Yii::t('app', 'Log cannot be created.'));
            }
        }
        ...
        if (isset($_POST['Log'])) {
            ...
            if ($model->validate()) {
                if ($model->save()) {
                    ...
                    throw new CHttpException(500, Yii::t('app', 'Log cannot be saved.'));
            }
        }
        $this->render('//log/update', array(
            'model' => $model,
            'dependency' => (isset($_GET['dependency'])?$_GET['dependency']:null), 
            'dependencyTabIndex' => (isset($_GET['dependencyTabIndex'])?$_GET['dependencyTabIndex']:null), 
            'dependencyTabDropdownIndex' => (isset($_GET['dependencyTabDropdownIndex'])?$_GET['dependencyTabDropdownIndex']:null),
    ...
    public function actionEdit()
    {
        if (isset($_POST['pk']) && isset($_POST['name']) && isset($_POST['value'])) {
            $model = Log::model()->findByPk((int)$_POST['pk']);
            if (isset($model)) {
    ...
    public function actionDelete($id)
    {
        if (Yii::app()->request->isPostRequest) {
            ...
            if (!isset($_REQUEST['ajax']))
                Yii::app()->user->setFlash('success', Yii::t('app', '<span class="label label-success">DELETED</span> <span class="label label-warning">:record</span> is deleted successfully.', array(':record' => $model->LOG_ID)));
            else
                echo Yii::t('app', '<div class="alert alert-success"><button type="button" class="close" data-dismiss="alert">&times;</button><span class="label label-success">DELETED</span> <span class="label label-warning">:record</span> is deleted successfully.</div>', array(':record' => $model->LOG_ID));
            // Redirect
            if (isset($_REQUEST['ajax']))
                Yii::app()->end();
    ...
    public function actionIndex()
    {
        ...
        if (isset($_GET['export'])) {
            if (Yii::app()->user->checkAccess('Log.Export')) {
                if (isset($_REQUEST['ajax']) && $_REQUEST['ajax'] === 'log-grid') {
                    header('Content-type: application/json');
                    $file = $model->export(false);
                    if ($file === false) {
                        $status = '500';
                        $statusText = 'fail';
                        $body = Yii::t('app', 'Log data cannot be exported.');
                    } else {
                        $status = '200';
                        $statusText = 'success';
                        $body = $file;
                    ...
                    if (!$model->export(true)) {
                        Yii::log('Log data cannot be exported.', CLogger::LEVEL_ERROR);
                        throw new CHttpException(500, Yii::t('app', 'Log data cannot be exported.'));
                    }
                }
            } else {
                Yii::log('This user is not permitted to export Log data.', CLogger::LEVEL_ERROR);
                throw new CHttpException(401, Yii::t('app', 'You are not fully authorized to export Log data.'));
            }
        }
...
//             TODO: Add additional filters here!
            if ($_GET['parentPk'] == 'ED1_ID')
                $model->ED1_ID = (int)$_GET['parentId'];
            if (isset($_GET['export'])) {
                if (Yii::app()->user->checkAccess('Log.Export')) {
                    if (isset($_REQUEST['ajax']) && $_REQUEST['ajax'] === 'log-grid-'.$_GET['dependencyTabDropdownIndex']) {
                        header('Content-type: application/json');
                        $file = $model->export(false);
                        if ($file === false) {
                            $status = '500';
    ...
    {
        header('Content-type: application/json');
        if (Yii::app()->user->checkAccess('Log.AjaxGet')) {
            $alias = Log::model()->getTableAlias(false, false);
            $criteria = new CDbCriteria();
            $criteria->params = array();
            // TODO: Add more relations here!
            if (isset($_GET['LOG_ID']))
                $criteria->compare($alias.'.LOG_ID', $_GET['LOG_ID']);
            // TODO: Add additional filters here!
            $return = array(
                'status' => '200',
                'statusText' => 'success',
                'body' => Log::getListData($criteria),
            );
        } else {
            $return = array(
                'status' => '401',
    ...
    {
        if (isset($_REQUEST['ajax']) && $_REQUEST['ajax'] === 'log-form') {
