...
class VendorController extends RController
{
    /**
     * REQUIRED
        ...
        if (isset($_POST['Vendor'])) {
            ...
            if ($model->validate()) {
                if ($model->save()) {
                    ...
                    throw new CHttpException(500, Yii::t('app', 'Vendor cannot be created.'));
            }
        }
        ...
        if (isset($_POST['Vendor'])) {
            ...
            if ($model->validate()) {
                if ($model->save()) {
                    ...
                    throw new CHttpException(500, Yii::t('app', 'Vendor cannot be saved.'));
            }
        }
    ...
    public function actionEdit()
    {
        if (isset($_POST['pk']) && isset($_POST['name']) && isset($_POST['value'])) {
            $model = Vendor::model()->findByPk((int)$_POST['pk']);
            if (isset($model)) {
    ...
    public function actionDelete($id)
    {
        if (Yii::app()->request->isPostRequest) {
            ...
            if (!isset($_REQUEST['ajax']))
                Yii::app()->user->setFlash('success', Yii::t('app', '<span class="label label-success">DELETED</span> <span class="label label-warning">:record</span> is deleted successfully.', array(':record' => $model->VD1_ID)));
            else
                echo Yii::t('app', '<div class="alert alert-success"><button type="button" class="close" data-dismiss="alert">&times;</button><span class="label label-success">DELETED</span> <span class="label label-warning">:record</span> is deleted successfully.</div>', array(':record' => $model->VD1_ID));
            // Redirect
            if (isset($_REQUEST['ajax']))
                Yii::app()->end();
    ...
    public function actionIndex()
    {
        ...
        $model->VD1_SHOW_DEFAULT = Vendor::SHOW_DEFAULT_FLAG;
        
        if (isset($_GET['Vendor']))
            $model->attributes = $_GET['Vendor'];
        if (isset($_GET['export'])) {
            if (Yii::app()->user->checkAccess('Vendor.Export')) {
                if (isset($_REQUEST['ajax']) && $_REQUEST['ajax'] === 'vendor-grid') {
                    header('Content-type: application/json');
                    $file = $model->export(false);
                    if ($file === false) {
                        $status = '500';
                        $statusText = 'fail';
                        $body = Yii::t('app', 'Vendor data cannot be exported.');
                    } else {
                        $status = '200';
                        $statusText = 'success';
                        $body = $file;
                    ...
                    if (!$model->export(true)) {
                        Yii::log('Vendor data cannot be exported.', CLogger::LEVEL_ERROR);
                        throw new CHttpException(500, Yii::t('app', 'Vendor data cannot be exported.'));
                    }
                }
            } else {
                Yii::log('This user is not permitted to export Vendor data.', CLogger::LEVEL_ERROR);
                throw new CHttpException(401, Yii::t('app', 'You are not fully authorized to export Vendor data.'));
            }
        }
        $this->render('//vendor/index', array(
            'model' => $model,
        ));
    }
    ...
    public function actionDependency()
    {
        if (isset($_GET['dependency']) && isset($_GET['parentPk']) && isset($_GET['parentId'])) {
            ...
            if (isset($_GET['export'])) {
                if (Yii::app()->user->checkAccess('Vendor.Export')) {
                    if (isset($_REQUEST['ajax']) && $_REQUEST['ajax'] === 'vendor-grid-'.$_GET['dependencyTabDropdownIndex']) {
                        header('Content-type: application/json');
                        $file = $model->export(false);
                        if ($file === false) {
                            $status = '500';
            ...
            $this->renderPartial('//vendor/_grid', array(
                'model' => $model,
    ...
    public function actionRelation()
    {
        ...
        if (isset($_GET['Vendor']))
            $model->attributes = $_GET['Vendor'];
        if (isset($_GET['VD1_ID'])) 
            $model->VD1_ID = $_GET['VD1_ID'];
        // TODO: Add additional filters here! 
        $this->renderPartial('//vendor/_grid', array(
            'model' => $model,
            'relation' => true,
    ...
    {
        header('Content-type: application/json');
        if (Yii::app()->user->checkAccess('Vendor.AjaxGet')) {
            $alias = Vendor::model()->getTableAlias(false, false);
            $criteria = new CDbCriteria();
            $criteria->params = array();
            // TODO: Add more relations here!
            if (isset($_GET['VD1_ID']))
                $criteria->compare($alias.'.VD1_ID', $_GET['VD1_ID']);
            // TODO: Add additional filters here!
            $return = array(
                'status' => '200',
                'statusText' => 'success',
                'body' => Vendor::getListData($criteria),
            );
        } else {
            $return = array(
                'status' => '401',
    ...
    {
        if (isset($_REQUEST['ajax']) && $_REQUEST['ajax'] === 'vendor-form') {
