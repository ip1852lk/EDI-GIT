...
class LocationController extends RController
{
    ...
    public function actionCreate()
    {
        ...
        $this->performAjaxValidation(array($model, $region));
        if (isset($_POST['Location']) && isset($_POST['Region'])) {
            $model->attributes = $_POST['Location'];
            $region->attributes = $_POST['Region'];
            if ($model->validate() && $region->validate()) {
                if ($model->save()) {
                    ...
                    throw new CHttpException(500, Yii::t('app', 'Location cannot be created.'));
            }
        }
        if (isset($_GET['dependency']) && isset($_GET['parentPk']) && isset($_GET['parentId'])) {
            if ($_GET['parentPk'] == 'rg1_id') {
                $model->rg1_id = (int)$_GET['parentId'];
                $region = Region::model()->findByPk((int)$_GET['parentId']);
                if (isset($region)) {
                    $region->setScenario('relation');
                    $company = isset($region->company) ? $region->company : new Company();
                    $company->setScenario('relation');
                    $model->us1_id = $region->us1_id;
                    $representative = Profile::model()->findByPk($model->us1_id);
                    if (!isset($representative)) $representative = new Profile();
            ...
            } elseif ($_GET['parentPk'] == 'co1_id') {
                $region->co1_id = (int)$_GET['parentId'];
    ...
    {
        $model = $this->loadModel($id);
        $region = isset($model->region) ? $model->region : new Region();
        $region->setScenario('relation');
        $company = isset($region->company) ? $region->company : new Company();
        $company->setScenario('relation');
        ...
        if (isset($_POST['Location']) && isset($_POST['Region'])) {
            ...
            if ($model->validate() && $region->validate()) {
                if ($model->save()) {
                    ...
                    throw new CHttpException(500, Yii::t('app', 'Location cannot be saved.'));
            }
        }
    ...
    public function actionDelete($id)
    {
        if (Yii::app()->request->isPostRequest) {
            ...
            if (!isset($_REQUEST['ajax']))
                Yii::app()->user->setFlash('success', Yii::t('app', '<span class="label label-success">DELETED</span> <span class="label label-warning">:record</span> is deleted successfully.', array(':record' => $model->lo1_code)));
            else
                echo Yii::t('app', '<div class="alert alert-success"><button type="button" class="close" data-dismiss="alert">&times;</button><span class="label label-success">DELETED</span> <span class="label label-warning">:record</span> is deleted successfully.</div>', array(':record' => $model->lo1_code));
            // Redirect
            if (isset($_REQUEST['ajax'])) 
                Yii::app()->end();
    ...
    public function actionIndex()
    {
        ...
        if (isset($_GET['export'])) {
            if (Yii::app()->user->checkAccess('Location.Export')) {
                if (isset($_REQUEST['ajax']) && $_REQUEST['ajax'] === 'location-grid') {
                    header('Content-type: application/json');
                    $file = $model->export(false);
                    if ($file === false) {
                        $status = '500';
                        $statusText = 'fail';
                        $body = Yii::t('app', 'Location data cannot be exported.');
                    } else {
                        $status = '200';
                        $statusText = 'success';
                        $body = $file;
                    ...
                    if (!$model->export(true)) {
                        Yii::log('Location data cannot be exported.', CLogger::LEVEL_ERROR);
                        throw new CHttpException(500, Yii::t('app', 'Location data cannot be exported.'));
                    }
                }
            } else {
                Yii::log('This user is not permitted to export Location data.', CLogger::LEVEL_ERROR);
                throw new CHttpException(401, Yii::t('app', 'You are not fully authorized to export Location data.'));
            }
        }
    ...
    public function actionDependency()
    {
        if (isset($_GET['dependency']) && isset($_GET['parentPk']) && isset($_GET['parentId'])) {
            ...
            if (isset($_GET['export'])) {
                if (Yii::app()->user->checkAccess('Location.Export')) {
                    if (isset($_REQUEST['ajax']) && $_REQUEST['ajax'] === 'location-grid-'.$_GET['dependencyTabDropdownIndex']) {
                        header('Content-type: application/json');
                        $file = $model->export(false);
                        if ($file === false) {
                            $status = '500';
            ...
            $this->renderPartial('//location/_grid', array(
                'model' => $model,
    ...
    {
        header('Content-type: application/json');
        if (Yii::app()->user->checkAccess('Location.AjaxGet')) {
            ...
            if (isset($_GET['lo1_id']))
                $criteria->compare($alias.'.id', $_GET['lo1_id']);
            if (isset($_GET['rg1_id']))
                $criteria->compare($alias.'.rg1_id', $_GET['rg1_id']);
            if (isset($_GET['co1_id']))
                $criteria->compare('region.co1_id', $_GET['co1_id']);
            $return = array(
                'status' => '200',
                'statusText' => 'success',
                'body' => Location::getListData($criteria),
            );
        } else {
            $return = array(
                'status' => '401',
    ...
    {
        if (isset($_REQUEST['ajax']) && $_REQUEST['ajax'] === 'location-form') {
