...
class ProjectController extends RController
{
    ...
    {
        $this->render('//project/view', array(
    ...
    public function actionCreate()
    {
        ...
        if (isset($_POST['Project'])) {
            ...
            if ($model->validate()) {
                if ($model->save()) {
                    ...
                    throw new CHttpException(500, Yii::t('app', 'Project cannot be created.'));
            }
        }
        ...
        $this->render('//project/create', array(
            'model' => $model,
        ...
        if (isset($_POST['Project'])) {
            ...
            if ($model->validate()) {
                if ($model->save()) {
                    ...
                    throw new CHttpException(500, Yii::t('app', 'Project cannot be saved.'));
            }
        }
        $this->render('//project/update', array(
            'model' => $model,
            'dependency' => (isset($_GET['dependency'])?$_GET['dependency']:null), 
            'dependencyTabIndex' => (isset($_GET['dependencyTabIndex'])?$_GET['dependencyTabIndex']:null), 
            'dependencyTabDropdownIndex' => (isset($_GET['dependencyTabDropdownIndex'])?$_GET['dependencyTabDropdownIndex']:null),
    ...
    public function actionEdit()
    {
        if (isset($_POST['pk']) && isset($_POST['name']) && isset($_POST['value'])) {
            $model = Project::model()->findByPk((int)$_POST['pk']);
            if (isset($model)) {
    ...
    public function actionBlog()
    {
        ...
        $this->render('//project/blog', array(
    ...
    public function actionIndex()
    {
        ...
        if (isset($_GET['export'])) {
            if (Yii::app()->user->checkAccess('Project.Export')) {
                if (isset($_REQUEST['ajax']) && $_REQUEST['ajax'] === 'project-grid') {
                    header('Content-type: application/json');
                    $file = $model->export(false);
                    if ($file === false) {
                        $status = '500';
                        $statusText = 'fail';
                        $body = Yii::t('app', 'Project data cannot be exported.');
                    } else {
                        $status = '200';
                        $statusText = 'success';
                        $body = $file;
                    ...
                    if (!$model->export(true)) {
                        Yii::log('Project data cannot be exported.', CLogger::LEVEL_ERROR);
                        throw new CHttpException(500, Yii::t('app', 'Project data cannot be exported.'));
                    }
                }
            } else {
                Yii::log('This user is not permitted to export Project data.', CLogger::LEVEL_ERROR);
                throw new CHttpException(401, Yii::t('app', 'You are not fully authorized to export Project data.'));
            }
        }
    ...
    public function actionDependency()
    {
        if (isset($_GET['dependency']) && isset($_GET['parentPk']) && isset($_GET['parentId'])) {
            ...
            if (isset($_GET['export'])) {
                if (Yii::app()->user->checkAccess('Project.Export')) {
                    if (isset($_REQUEST['ajax']) && $_REQUEST['ajax'] === 'project-grid-'.$_GET['dependencyTabDropdownIndex']) {
                        header('Content-type: application/json');
                        $file = $model->export(false);
                        if ($file === false) {
                            $status = '500';
            ...
            $this->renderPartial('//project/_grid', array(
                'model' => $model,
    ...
    {
        header('Content-type: application/json');
        if (Yii::app()->user->checkAccess('Project.AjaxGet')) {
            $alias = Project::model()->getTableAlias(false, false);
            ...
            $return = array(
                'status' => '200',
                'statusText' => 'success',
                'body' => Project::getListData($criteria),
            );
        } else {
            $return = array(
                'status' => '401',
     ...
     * Returns all models in JSON.
     */
    public function actionAjaxGetInvoiceType()
    {
        header('Content-type: application/json');
        if (isset($_GET['pr1_id'])) {
            $project = Project::model()->findByPk($_GET['pr1_id']);
            if(isset($project->RE1_INVOICE_TYPE,$project->RE1_INVOICE_BILLED) && $project->RE1_INVOICE_TYPE>=0 && $project->RE1_INVOICE_BILLED>=0 ){
                $return = array(
                    'status' => '200',
                    'statusText' => 'success',
                    'invoiceType' => $project->RE1_INVOICE_TYPE,
                    'invoiceBilled' => $project->RE1_INVOICE_BILLED,
                );
            }else {
                $return = array(
                    'status' => '401',
    ...
    {
        if (isset($_REQUEST['ajax']) && $_REQUEST['ajax'] === 'project-form') {
