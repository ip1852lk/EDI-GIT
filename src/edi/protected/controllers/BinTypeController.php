...
class BinTypeController extends RController
{
    /**
     * REQUIRED
    ...
    {
        $this->render('//binType/view', array(
    ...
    public function actionCreate()
    {
        ...
        if (isset($_POST['BinType'])) {
            $model->attributes = $_POST['BinType'];
            if ($model->validate()) {
                if ($model->save()) {
                    ...
                    throw new CHttpException(500, Yii::t('app', 'BinType cannot be created.'));
            }
        }
        ...
        $this->render('//binType/create', array(
            'model' => $model,
        ...
        if (isset($_POST['BinType'])) {
            ...
            if ($model->validate()) {
                if ($model->save()) {
                    ...
                    throw new CHttpException(500, Yii::t('app', 'BinType cannot be saved.'));
            }
        }
        $this->render('//binType/update', array(
            'model' => $model,
            'dependency' => (isset($_GET['dependency'])?$_GET['dependency']:null), 
            'dependencyTabIndex' => (isset($_GET['dependencyTabIndex'])?$_GET['dependencyTabIndex']:null), 
            'dependencyTabDropdownIndex' => (isset($_GET['dependencyTabDropdownIndex'])?$_GET['dependencyTabDropdownIndex']:null),
    ...
    public function actionEdit()
    {
        if (isset($_POST['pk']) && isset($_POST['name']) && isset($_POST['value'])) {
            $model = BinType::model()->findByPk((int)$_POST['pk']);
            if (isset($model)) {
    ...
    {
        $dataProvider = new CActiveDataProvider('BinType');
        $this->render('//binType/blog', array(
    ...
    public function actionIndex()
    {
        ...
        $model = new BinType('search');
        $model->unsetAttributes();  // clear any default values
        if (isset($_GET['BinType']))
            $model->attributes = $_GET['BinType'];
        if (isset($_GET['export'])) {
            if (Yii::app()->user->checkAccess('BinType.Export')) {
                if (isset($_REQUEST['ajax']) && $_REQUEST['ajax'] === 'bin-type-grid') {
                    header('Content-type: application/json');
                    $file = $model->export(false);
                    if ($file === false) {
                        $status = '500';
                        $statusText = 'fail';
                        $body = Yii::t('app', 'BinType data cannot be exported.');
                    } else {
                        $status = '200';
                        $statusText = 'success';
                        $body = $file;
                    ...
                    if (!$model->export(true)) {
                        Yii::log('BinType data cannot be exported.', CLogger::LEVEL_ERROR);
                        throw new CHttpException(500, Yii::t('app', 'BinType data cannot be exported.'));
                    }
                }
            } else {
                Yii::log('This user is not permitted to export BinType data.', CLogger::LEVEL_ERROR);
                throw new CHttpException(401, Yii::t('app', 'You are not fully authorized to export BinType data.'));
            }
        }
        $this->render('//binType/index', array(
            'model' => $model,
        ));
    }
    ...
    public function actionDependency()
    {
        if (isset($_GET['dependency']) && isset($_GET['parentPk']) && isset($_GET['parentId'])) {
            ...
            if (isset($_GET['export'])) {
                if (Yii::app()->user->checkAccess('BinType.Export')) {
                    if (isset($_REQUEST['ajax']) && $_REQUEST['ajax'] === 'bin-type-grid-'.$_GET['dependencyTabDropdownIndex']) {
                        header('Content-type: application/json');
                        $file = $model->export(false);
                        if ($file === false) {
                            $status = '500';
            ...
            $this->renderPartial('//binType/_grid', array(
                'model' => $model,
    ...
    {
        header('Content-type: application/json');
        if (Yii::app()->user->checkAccess('BinType.AjaxGet')) {
            ...
            $return = array(
                'status' => '200',
                'statusText' => 'success',
                'body' => BinType::getListData($criteria),
            );
        } else {
            $return = array(
                'status' => '401',
    ...
    {
        $model = BinType::model()->findByPk($id);
        if ($model === null)
            throw new CHttpException(404, Yii::t('app', 'The requested page does not exist.'));
        return $model;
    }
    ...
    {
        if (isset($_REQUEST['ajax']) && $_REQUEST['ajax'] === 'bin-type-form') {
