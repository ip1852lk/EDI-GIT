...
class Rights
{
    ...
    /**
     * Returns the list of authorization item icons.
     * @return array the list of types.
     */
    public static function getAuthItemIcons()
    {
        return array(
            CAuthItem::TYPE_OPERATION => 'fa fa-legal',
            CAuthItem::TYPE_TASK => 'fa fa-tasks',
            CAuthItem::TYPE_ROLE => 'fa fa-users',
        );
    }

    /**
     * Returns the name of a specific authorization item icon.
     * @param integer $type the item type (0: operation, 1: task, 2: role).
     * @return string the authorization item icon.
     */
    public static function getAuthItemIcon($type)
    {
        $icons = self::getAuthItemIcons();
        if (isset($icons[$type]) === true)
            return $icons[$type];
        else
            throw new CException(Rights::t('core', 'Invalid authorization item type.'));
    }
