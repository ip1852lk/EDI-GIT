...
class UserController extends RController
{
            ...
            if ($model->validate() && $profile->validate()) {
                ...
                if ($model->save()) {
                    ...
                    if ($profile->save()) {
                        ...
                        elseif ($profile->user_type == User::TYPE_INTERNAL_SUPERVISOR)
                            Rights::getAuthorizer()->authManager->assign('Internal.Supervisor', $model->id);
                        elseif ($profile->user_type == User::TYPE_INTERNAL_USER)
                            Rights::getAuthorizer()->authManager->assign('Internal.User', $model->id);
    ...
    public function actionRelation()
    {
        ...
        if (isset($_GET['cu1_id']))
            $criteria->compare('profile.cu1_id', (int)$_GET['cu1_id']);
        if (isset($_GET['parentPk']) && $_GET['parentPk'] == 'su1_id') 
            $criteria->compare('profile.su1_id', (int)$_GET['parentId']);
        if (isset($_GET['su1_id']))
            $criteria->compare('profile.cu1_id', (int)$_GET['su1_id']);
        $model->setDbCriteria($criteria);
        $this->renderPartial('application.modules.user.views.user._grid', array(
            'model' => $model,
            'relation' => true,
