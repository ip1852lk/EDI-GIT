...
class RAuthorizer extends CApplicationComponent
{
    ...
    public function getAuthItems($types = null, $userId = null, CAuthItem $parent = null, $sort = true, $exclude = array())
    {
        ...
        // We have multiple types.
            ...
            $items = array();
            if (array_key_exists(CAuthItem::TYPE_ROLE, $types)) {
                $items = $this->mergeAuthItems($items, $typeItemList[CAuthItem::TYPE_ROLE]);
            }
            if (array_key_exists(CAuthItem::TYPE_TASK, $types) && array_key_exists(CAuthItem::TYPE_OPERATION, $types)) {
                foreach ($typeItemList[CAuthItem::TYPE_TASK] as $itemName => $item) {
                    if (!isset($items[$itemName])) {
                        $items[$itemName] = $item;
                        $itemName = str_replace('*', '', $itemName);
                        foreach ($typeItemList[CAuthItem::TYPE_OPERATION] as $operationName => $operationValue) {
                            if (!isset($items[$operationName]) && strpos($operationName, $itemName) === 0)
                                $items[$operationName] = $operationValue;
                        }
                    }
                }
            }
            $items = $this->mergeAuthItems($items, $typeItemList[CAuthItem::TYPE_OPERATION]);
        }
    ...
    public function getSuperusers()
    {
        ...
        $criteria->alias = CActiveRecord::model($userClass)->getTableAlias(false, false);
        $criteria->addInCondition($criteria->alias . '.' . Rights::module()->userIdColumn, $userIdList);
