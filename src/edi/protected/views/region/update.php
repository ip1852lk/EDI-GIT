...
// Title
$this->title = Yii::t('app', 'Region').' <span class="text-warning">'.$model->rg1_code.'</span>';
// Breadcrumbs
$this->breadcrumbs = array(
    Yii::t('app', Yii::app()->params['workspaceLabel']) => array(Yii::app()->params['workspaceUrl']),
    Yii::t('app', 'Regions') => array('index'),
    $model->rg1_code => array('view', 'id' => $model->id),
    Yii::t('app', 'Update'),
);
// Menus
...
$this->menu = array_merge($this->menu, array(
    array(
        ...
        'url' => '#',
        'encodeLabel' => false,
        'htmlOptions' => array('id' => 'region-update-save-btn', 'class' => 'navbar-btn btn-sm',),
    ),
));
if ($regionDelete) {
    $this->menu = array_merge($this->menu, array(
        array(
            ...
            'url' => '#',
            'encodeLabel' => false,
            'htmlOptions' => array('class' => 'region-delete-btn navbar-btn btn-sm',),
            'visible' => $regionDelete,
        ),
    ));
    if (isset($dependency))
        $deleteUrl = array(
    ...
    $cs->registerScript(__CLASS__ . 'region_delete', '
            ...
            bootbox.dialog({
                ...
                buttons: {
                    delete:{label:"' . Yii::t('app', 'Delete') . '", className:"btn-danger", callback:function(){
                        $.yii.submitForm($(".region-delete-btn")[0], "' . $this->createUrl('delete', $deleteUrl) . '", {YII_CSRF_TOKEN:"' . Yii::app()->request->csrfToken . '"});
                    }},
...
if ($this->isMobile) {
    $this->widget('booster.widgets.TbTabs', array(
        'type' => 'tabs',
        'tabMenuHtmlOptions' => array('id' => 'region_form_tab_menu'),
        'tabs' => array(
            array(
                'active' => isset($tabIndex) && $tabIndex==0 ? true : false,
                'id' => 'region-form-tab',
                'label' => Yii::t('app', 'Region'),
                'content' => $this->renderPartial('//region/_form', array(
                    'model' => $model,
                    'company' => $company,
    ...
    <div id="region-form-content" class="row">
        <div class="col-sm-12 col-md-5">
            <?php
            $this->renderPartial('//region/_form', array(
                'model' => $model,
                'company' => $company,
                'representative' => $representative,
                'dependency' => (isset($dependency)?$dependency:null),
        ...
        <div id="region-dependency-content" class="col-sm-12 col-md-7">
            <?php
            $this->widget('booster.widgets.TbTabs', array(
                'type' => 'tabs',
                'tabMenuHtmlOptions' => array('id' => 'region_form_tab_menu', 'class' => 'dependency-dropdown'),
                'tabs' => array(
                    array(
                        'active' => isset($tabIndex) && $tabIndex==1 ? true : false,
                        'label' => Yii::t('app', 'Dependency Lists'),
...
if ($tabIndex == 1 && $tabDropdownIndex == 0) 
    $cs->registerScript(__CLASS__ . 'region_location_control', $locationList);
if ($tabIndex == 1 && $tabDropdownIndex == 1)
    $cs->registerScript(__CLASS__ . 'region_table_log_control', $tableLogList);
// Tab Events
$cs->registerScript(__CLASS__ . 'region_dependency_control', '
    $("#region-location-tab-btn").click(function(){'.$locationList.'});
    $("#region-table-log-tab-btn").click(function(){'.$tableLogList.'});
');
// Save Control
$cs->registerScript(__CLASS__ . 'region_form_save', '
    $("#region-update-save-btn").click(function(){
        $("#region-form-save-btn").trigger("click")
    });
');
