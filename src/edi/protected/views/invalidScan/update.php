...
// Title
$this->title = Yii::t('app', 'Invalid Scan').' <span class="text-warning">'.$model->id.'</span>';
// Breadcrumbs
...
$this->menu = array_merge($this->menu, array(
    array(
        ...
        'url' => '#',
        'encodeLabel' => false,
        'htmlOptions' => array('id' => 'invalid-scan-update-save-btn', 'class' => 'navbar-btn btn-sm',),
    ),
));
...
$tableLogList = 'window.getDependencyGrid(
    "'.$this->createUrl('/tableLog/dependency', array(
        'dependency' => '/invalidScan/update',
        'dependencyTabIndex' => 1,
        'dependencyTabDropdownIndex' => 0,
        'parentPk' => 'InvalidScan',
        'parentId' => $model->id,
    )).'",
    "invalid-scan-table-log-tab",
    "'.Yii::t('app', 'Loading...').'",
...
if ($this->isMobile) {
    $this->widget('booster.widgets.TbTabs', array(
        'type' => 'tabs',
        'tabMenuHtmlOptions' => array('id' => 'invalid-scan_form_tab_menu'),
        'tabs' => array(
            array(
                'active' => isset($tabIndex) && $tabIndex==0 ? true : false,
                'id' => 'invalid-scan-form-tab',
                'label' => Yii::t('app', 'InvalidScan'),
                'content' => $this->renderPartial('//invalidScan/_form', array(
                    'model' => $model,
    ...
    <div id="invalid-scan-form-content" class="row">
        <div class="col-sm-12 col-md-5">
            <?php
            $this->renderPartial('//invalidScan/_form', array(
                'model' => $model,
                'dependency' => (isset($dependency)?$dependency:null),
                'dependencyTabIndex' => (isset($dependencyTabIndex)?$dependencyTabIndex:null),
                'dependencyTabDropdownIndex' => (isset($dependencyTabDropdownIndex)?$dependencyTabDropdownIndex:null),
        ...
        <div id="invalid-scan-dependency-content" class="col-sm-12 col-md-7">
            <?php
            $this->widget('booster.widgets.TbTabs', array(
                'type' => 'tabs',
                'tabMenuHtmlOptions' => array('id' => 'invalid-scan_form_tab_menu', 'class' => 'dependency-dropdown'),
                'tabs' => array(
                    array(
                        'active' => isset($tabIndex) && $tabIndex==1 ? true : false,
                        'label' => Yii::t('app', 'Dependency Lists'),
...
if ($tabIndex == 1 && $tabDropdownIndex == 0) 
    $cs->registerScript(__CLASS__ . 'invalid_scan_table_log_control', $tableLogList);
// Tab Events
$cs->registerScript(__CLASS__ . 'invalid_scan_dependency_control', '
    $("#invalid-scan-table-log-tab-btn").click(function(){'.$tableLogList.'});
');
// Save Control
$cs->registerScript(__CLASS__ . 'invalid_scan_form_save', '
    $("#invalid-scan-update-save-btn").click(function(){
        $("#invalid-scan-form-save-btn").trigger("click")
    });
');
