...
$invalidScanView = Yii::app()->user->checkAccess('InvalidScan.View');
...
if (isset($dependency) || isset($relation)) {
    ...
    echo $this->renderPartial('//invalidScan/_grid_menu', array(
        'model' => $model,
...
// Status Message
if (!isset($relation)) {
    echo '<div class="invalid-scan-grid-status-msg">';
...
if (isset($relation)) {
    $columns = array(
        ...
        array(
            ...
            'value' => 
                '"id==".$data->id'
                .'."|is1_error_code==".$data->is1_error_code'
                .'."|us1_id==".$data->us1_id'
                .'."|cu1_id==".$data->cu1_id'
                .'."|or1_type==".$data->or1_type'
                .'."|contract_bin_id==".$data->contract_bin_id'
                .'."|or2_scanned_qty==".$data->or2_scanned_qty',
    ...
    $columns = array(
        array(
            'name' => 'id',
            'type' => 'raw',
            'value' => $invalidScanUpdate ? 'TbHtml::link(UHtml::markSearch($data, "id"), array('.$updateLink.'))' : '$data->id',
        ),
        'is1_error_code',
        'us1_id',
        'cu1_id',
        ...
        array(
            ...
            'visible' => !isset($dependency) && $invalidScanAdmin,
        ),
        array(
            'name' => 'modified_on',
            ...
            'filter' => $this->widget('booster.widgets.TbDatePicker', array(
                ...
                'name' => 'InvalidScan[modified_on]',
                'htmlOptions' => array(
                    'id' => 'InvalidScan_modified_on'.(isset($dependency)?'_'.$dependencyTabDropdownIndex:''),
                    'class' => 'form-control',
        ...
        array(
            'header' => TbHtml::dropDownList(
                ...
                array(
                    'onchange' => "$.fn.yiiGridView.update('".(isset($dependency)?'invalid-scan-grid-'.$dependencyTabDropdownIndex:'invalid-scan-grid')."', {data:{pageSize:$(this).val()}})",
                )
            ),
            'class' => 'booster.widgets.TbButtonColumn',
            'template' => $invalidScanDelete?'{delete}':'', //($invalidScanView?'{view} ':'').($invalidScanUpdate?'{update} ':'').($invalidScanDelete?'{delete}':''),
            'htmlOptions' => array('style' => 'width: 75px; text-align: center;'),
            'visible' => $invalidScanDelete, //$invalidScanView || $invalidScanUpdate || $invalidScanDelete,
            'buttons' => array(
                'view' => array(
                    'icon' => 'fa fa-lg fa-eye',
                    'url' => 'array("/invalidScan/view", "id" => $data->id)',
                    'options' => array('title' => Yii::t('app', 'View')),
                ),
                'update' => array(
                    'icon' => 'fa fa-lg fa-pencil',
                    'url' => 'array("/invalidScan/update", "id" => $data->id)',
                    'options' => array('title' => Yii::t('app', 'Update')),
                ),
                'delete' => array(
                    'icon' => 'fa fa-lg fa-trash-o',
                    'url' => 'array("/invalidScan/delete", "id" => $data->id)',
                    'options' => array('title' => Yii::t('app', 'Delete')),
                    'click' => 'function(){ 
                        var th = this;
                        var afterDelete = function(link,success,data){ $(".invalid-scan-grid-status-msg").html(data); };
                        bootbox.dialog({
                            ...
                            buttons: {
                                "delete":{label:"' . Yii::t('app', 'Delete') . '", className:"btn-danger", callback:function(){ 
                                    $("#'.(isset($dependency)?'invalid-scan-grid-'.$dependencyTabDropdownIndex:'invalid-scan-grid').'").yiiGridView("update", {
                                        type: "POST",
                                        url: $(th).attr("href"),
...
$this->widget('ext.jgridview.JGridView', array(
    'id' => (isset($relation)?'invalid-scan-grid-'.$relationIndex:(isset($dependency)?'invalid-scan-grid-'.$dependencyTabDropdownIndex:'invalid-scan-grid')),
