...
$supplierView = Yii::app()->user->checkAccess('Supplier.View');
...
if (isset($dependency) || isset($relation)) {
    ...
    echo $this->renderPartial('//supplier/_grid_menu', array(
        'model' => $model,
...
// Status Message
if (!isset($relation)) {
    echo '<div class="supplier-grid-status-msg">';
...
if (isset($relation)) {
    $columns = array(
        ...
        array(
            ...
            'value' => 
                '"id==".$data->id."|'.
                'su1_code==".$data->su1_code."|'.
                'su1_name==".$data->su1_name',
    ...
    $columns = array(
        ...
        array(
            'name' => 'su1_name',
            'type' => 'raw',
            'value' => $supplierUpdate ? 'TbHtml::link(UHtml::markSearch($data, "su1_name"), array('.$updateLink.'))' : '$data->su1_name',
        ),
        array(
            'name' => 'su1_phone',
        ...
        array(
            ...
            'visible' => !isset($dependency) && $supplierAdmin,
        ),
        array(
            'name' => 'modified_on',
            ...
            'filter' => $this->widget('booster.widgets.TbDatePicker', array(
                ...
                'name' => 'Supplier[modified_on]',
                'htmlOptions' => array(
                    'id' => 'Supplier_modified_on'.(isset($dependency)?'_'.$dependencyTabDropdownIndex:''),
                    'class' => 'form-control',
        ...
        array(
            'header' => TbHtml::dropDownList(
                ...
                array(
                    'onchange' => "$.fn.yiiGridView.update('".(isset($dependency)?'supplier-grid-'.$dependencyTabDropdownIndex:'supplier-grid')."', {data:{pageSize:$(this).val()}})",
                )
            ),
            'class' => 'booster.widgets.TbButtonColumn',
            'template' => $supplierDelete?'{delete}':'', //($supplierView?'{view} ':'').($supplierUpdate?'{update} ':'').($supplierDelete?'{delete}':''),
            'htmlOptions' => array('style' => 'width: 75px; text-align: center;'),
            'visible' => $supplierDelete, //$supplierView || $supplierUpdate || $supplierDelete,
            'buttons' => array(
                'view' => array(
                    'icon' => 'fa fa-lg fa-eye',
                    'url' => 'array("/supplier/view", "id" => $data->id)',
                    'options' => array('title' => Yii::t('app', 'View')),
                ),
                'update' => array(
                    'icon' => 'fa fa-lg fa-pencil',
                    'url' => 'array("/supplier/update", "id" => $data->id)',
                    'options' => array('title' => Yii::t('app', 'Update')),
                ),
                'delete' => array(
                    'icon' => 'fa fa-lg fa-trash-o',
                    'url' => 'array("/supplier/delete", "id" => $data->id)',
                    'options' => array('title' => Yii::t('app', 'Delete')),
                    'click' => 'function(){ 
                        var th = this;
                        var afterDelete = function(link,success,data){ $(".supplier-grid-status-msg").html(data); };
                        bootbox.dialog({
                            ...
                            buttons: {
                                "delete":{label:"' . Yii::t('app', 'Delete') . '", className:"btn-danger", callback:function(){ 
                                    $("#'.(isset($dependency)?'supplier-grid-'.$dependencyTabDropdownIndex:'supplier-grid').'").yiiGridView("update", {
                                        type: "POST",
                                        url: $(th).attr("href"),
...
$this->widget('ext.jgridview.JGridView', array(
    'id' => (isset($relation)?'supplier-grid-'.$relationIndex:(isset($dependency)?'supplier-grid-'.$dependencyTabDropdownIndex:'supplier-grid')),
