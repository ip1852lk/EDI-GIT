...
$this->breadcrumbs = array(
    ...
    $model->lo1_code => array('view', 'id' => $model->id),
    Yii::t('app', 'Update'),
);
// Menus
...
$this->menu = array_merge($this->menu, array(
    array(
        ...
        'url' => '#',
        'encodeLabel' => false,
        'htmlOptions' => array('id' => 'location-update-save-btn', 'class' => 'navbar-btn btn-sm',),
    ),
));
if ($locationDelete) {
    $this->menu = array_merge($this->menu, array(
...
// Tab Control
$executiveMgtList = 'window.getDependencyGrid(
    "'.$this->createUrl('/executiveMgt/dependency', array(
        'dependency' => '/location/update',
        'dependencyTabIndex' => 1,
        'dependencyTabDropdownIndex' => 0,
        'parentPk' => 'lo1_id',
        'parentId' => $model->id,
    )).'",
    "location-executive-mgt-tab",
    "'.Yii::t('app', 'Loading...').'",
...
if ($this->isMobile) {
    $this->widget('booster.widgets.TbTabs', array(
        'type' => 'tabs',
        'tabMenuHtmlOptions' => array('id' => 'location_form_tab_menu'),
        'tabs' => array(
            array(
                'active' => isset($tabIndex) && $tabIndex==0 ? true : false,
                'id' => 'location-form-tab',
                'label' => Yii::t('app', 'Location'),
                'content' => $this->renderPartial('//location/_form', array(
                    'model' => $model,
                    'region' => $region,
                    'company' => $company,
    ...
    <div id="location-form-content" class="row">
        <div class="col-sm-12 col-md-5">
            <?php
            $this->renderPartial('//location/_form', array(
                'model' => $model,
                'region' => $region,
                'company' => $company,
                'representative' => $representative,
        ...
        <div id="location-dependency-content" class="col-sm-12 col-md-7">
            <?php
            $this->widget('booster.widgets.TbTabs', array(
                'type' => 'tabs',
                'tabMenuHtmlOptions' => array('id' => 'location_form_tab_menu', 'class' => 'dependency-dropdown'),
                'tabs' => array(
                    array(
                        'active' => isset($tabIndex) && $tabIndex==1 ? true : false,
                        'label' => Yii::t('app', 'Dependency Lists'),
...
if ($tabIndex == 1 && $tabDropdownIndex == 0)
    $cs->registerScript(__CLASS__ . 'location_executive_mgt_control', $executiveMgtList);
elseif ($tabIndex == 1 && $tabDropdownIndex == 1)
    $cs->registerScript(__CLASS__ . 'location_user_control', $userList);
elseif ($tabIndex == 1 && $tabDropdownIndex == 2)
    $cs->registerScript(__CLASS__ . 'location_table_log_control', $tableLogList);
// Tab Events
$cs->registerScript(__CLASS__ . 'location_dependency_control', '
    $("#location-executive-mgt-tab-btn").click(function(){'.$executiveMgtList.'});
    $("#location-user-tab-btn").click(function(){'.$userList.'});
    $("#location-table-log-tab-btn").click(function(){'.$tableLogList.'});
');
// Save Control
$cs->registerScript(__CLASS__ . 'location_form_save', '
    $("#location-update-save-btn").click(function(){
